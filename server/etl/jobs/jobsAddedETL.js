...
/**
 * 📡 Fetch metric data for a single date
 */
...
export async function runJobsAddedETL(startDate, endDate) {
  ...
  try {
    ...
    for (const date of dates) {
      ...
      if (!row) continue;  // Skip invalid/null rows

      await publishMetricEvent(row);
      console.log(`📤 Published Kafka event → jobsadded @ ${row.metric_date}`);
    }
